...
exports.showRepoSwitcher = showRepoSwitcher;
...
async function scanForGitRepos(dir, depth, maxDepth, results) {
    if (depth > maxDepth)
        return;
    let entries;
    try {
 ...
 */
async function showRepoSwitcher() {
    ...
    const recent = (await vscode.commands.executeCommand('vscode.getRecentlyOpenedWorkspaces').then((r) => r?.workspaces ?? [], () => []));
    ...
    if (localItems.length > 0)
        items.push(localSep, ...localItems);
    // 3. GitHub repos
    ...
    if (githubItems.length > 0)
        items.push(githubSep, ...githubItems);
    // Empty state
    ...
    qp.onDidAccept(async () => {
        ...
        if (selected.itemKind === 'local' && selected.localPath) {
            await vscode.commands.executeCommand('vscode.openFolder', vscode.Uri.file(selected.localPath));
            return;
        }
        if (selected.itemKind === 'github' && selected.cloneUrl) {
            ...
            await vscode.window.withProgress({
                ...
                try {
                    ...
                    await vscode.commands.executeCommand('vscode.openFolder', vscode.Uri.file(inputUri));
                }
                catch (err) {
                    const e = err;
                    vscode.window.showErrorMessage(`Couldn't download the project: ${e.message ?? 'Unknown error'}`);
                }
            });
        }
    });
...
//# sourceMappingURL=repoSwitcher.js.map