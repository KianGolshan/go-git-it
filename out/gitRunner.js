...
exports.mergeExperiment = mergeExperiment;
exports.abandonExperiment = abandonExperiment;
 ...
 */
async function takeSnapshot(cwd, summary) {
...
async function pullLatest(cwd) {
    ...
    if (status.stdout) {
        return {
            ok: false,
            message: "You have unsaved changes — downloading new changes now could overwrite your work. Take a snapshot first!",
            code: 'DIRTY_PULL',
        };
    }
    await git(['fetch'], cwd);
 ...
 */
async function createExperiment(cwd, name) {
    ...
    return {
        ok: true,
        message: "You're now experimenting safely 🧪 — nothing here affects your main work until you're ready.",
    };
}
/**
 ...
 */
async function mergeExperiment(cwd, defaultBranch) {
    ...
    if (merge.failed) {
        ...
        if (stderr.includes('conflict') || merge.stdout.toLowerCase().includes('conflict')) {
            return {
                ok: false,
                message: "Two sets of changes touched the same part of a file, and we can't automatically decide which to keep. Your work is completely safe.",
                code: 'MERGE_CONFLICT',
 ...
 */
async function abandonExperiment(cwd, defaultBranch) {
    const branchResult = await git(['rev-parse', '--abbrev-ref', 'HEAD'], cwd);
    if (branchResult.failed || !branchResult.stdout.startsWith('experiment/')) {
        return {
            ok: false,
...
async function getState(cwd) {
    ...
    const headResult = await git(['symbolic-ref', 'refs/remotes/origin/HEAD'], cwd);
    if (!headResult.failed && headResult.stdout) {
        ...
        if (!masterCheck.failed)
            defaultBranch = 'master';
    }
    // Commits (last 20)
    const logResult = await git(['log', '--oneline', '--decorate', '--max-count=20', '--format=%H|%h|%s|%ci|%D'], cwd);
    const commits = [];
    if (!logResult.failed && logResult.stdout) {
        ...
        const remoteHashes = new Set(remoteHashesResult.failed ? [] : remoteHashesResult.stdout.split('\n').filter(Boolean));
        ...
        if (currentBranch.startsWith('experiment/')) {
            ...
            const mainHashes = new Set(mainLogResult.failed ? [] : mainLogResult.stdout.split('\n').filter(Boolean));
            let marked = false;
 ...
 */
async function initProject(projectPath, name) {
