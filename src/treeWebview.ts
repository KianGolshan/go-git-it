...
import { GitState, CommitNode } from './gitRunner'
...
function renderNode(c: CommitNode, isExperimentCol = false): string {
...
function buildHtml(state: GitState, nonce: string): string {
...
export class TreeWebviewProvider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(webviewView: vscode.WebviewView): void {
    ...
    webviewView.webview.onDidReceiveMessage(
      (msg: { type: string; hash?: string }) => {
        if (msg.type === 'commitClick' && msg.hash) this._onCommitClick(msg.hash)
      },
      undefined,
  ...
  update(state: GitState): void {
    this._state = state
    if (this._view?.visible) this._render(state)
  }

  private _render(state: GitState): void {
